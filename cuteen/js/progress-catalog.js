function TOC(opts) {
    ...
    window.addEventListener('scroll', coolScrollHandler, false)
    ...
    function coolScrollHandler() {
        ...
        tocItems.forEach(function (liItem) {
            ...
            if (bottom > wrapHeight * Opt.topMargin && top < wrapHeight * (1 - Opt.bottomMargin)) {
                ...
                firstChild.classList.add('cl-transform')
            } else {
                firstChild.classList.remove(Opt.linkActiveClass)
                firstChild.classList.remove('cl-transform')
            }
        })
