$(document).ready(function() {
    ...
    //初始化搜索引擎
    var current = localStorage.getItem("5iux-sou-search") || "baidu";
    $lg = $(".lg");
    $lg.addClass(current);
    //监听点击事件
    $lg.click(function() {
        if(current == 'baidu'){
            $lg.removeClass('baidu').addClass('google');
            current = 'google';
        }else{
            $lg.removeClass('google').addClass('baidu');
            current = 'baidu';
        }
    ...
    $('#search').submit(function(event) {
        ...
        if(current == 'baidu'){
            window.open('https://www.baidu.com/s?ie=utf-8&word=' + searchTerm);
        }else{
            window.open('https://www.google.com.hk/search?hl=zh&q=' + searchTerm);
        }
    });
});
//关键词sug
